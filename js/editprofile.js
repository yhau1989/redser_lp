...
function loadDataUser() {
    ...
    if (userLogon.length > 0) {
        var urlApi = `http://190.15.128.150:8030/resdec/user_get/?username=${userLogon}`;
...
function save() {
    ...
    var userLogon = sessionStorage.getItem("UserLoginResdec");
    var url = `http://190.15.128.150:8030/resdec/user_put/?username=${userLogon}&first_name=${nform}&last_name=${lnform}&email=${eform}`;
...
function savepws() {
    ...
    if (passform === repassform) {
        var userLogon = sessionStorage.getItem("UserLoginResdec");
        var url = `http://190.15.128.150:8030/resdec/user_upd_pass/?username=${userLogon}&password=${passform}`;
...
function saveImg() {
    ...
    $.ajax({
        ...
        contentType: false,
        processData: false,
        url: 'http://190.15.128.150:8030/resdec/user_photo_upload/',
        data: formData,
